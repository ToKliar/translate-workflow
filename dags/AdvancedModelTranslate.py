...
t1 = InputOperator(task_id = "1", file_paths=file_names, model = model, text_name=text_name, dag=dag, output_slot=pargraph_slot)
t2 = SegmentOperator(task_id = "2", dag=dag, input_slots=[pargraph_slot], output_slot=segment_slot)
t3 = ExtractOperator(task_id = "3", dag=dag, title="term", model = model, input_slots=[segment_slot], output_slot=word_slot)
t4 = WordTranslateOperator(task_id = "4", dag=dag, model = model, correct = True, input_slots=[word_slot], output_slot=word_slot)
...
t7 = TranslateOperator(task_id = "7", dag=dag, model = model, use_translation_model=False, input_slots=[segment_slot, match_slot], output_slot=segment_slot)
t8 = BackTranslationOperator(task_id = "8", model = model, dag=dag, input_slots=[segment_slot], output_slot=segment_slot)
t9 = OutputOperator(task_id = "9", dag=dag, file_path="/opt/airflow/dags/financial_max_advanced.txt", input_slots=[segment_slot])
