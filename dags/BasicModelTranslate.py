...
dag = DAG('basic_model_translate', default_args=default_args, schedule_interval='@once')

paragraph_slot = DataSlot(slot_id=f"basic_model", type_name=ParagraphHandler.type_name)
segment_slot = DataSlot(slot_id=f"basic_model", type_name=TranslationSegmentHandler.type_name)
...
t2 = SegmentOperator(task_id = "2", dag=dag, config=segment_config, input_slots=[paragraph_slot], output_slot=segment_slot)
t3 = TranslateOperator(task_id="3", dag=dag, model="qwen-mt-turbo", use_translation_model=True, input_slots=[segment_slot], output_slot=segment_slot)
t4 = OutputOperator(task_id = "4", dag=dag, file_path="/opt/airflow/dags/financial_model.txt", input_slots=[segment_slot])
